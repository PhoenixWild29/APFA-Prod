...
class TestAbsoluteSessionMax:
    def test_session_within_max_is_allowed(self, db, test_user):
        """Session younger than the absolute maximum should rotate normally."""
        raw, _ = create_refresh_token(db, test_user.id)
        db.commit()

        new_raw, _, user_id = rotate_refresh_token(db, raw)
        db.commit()
    ...
    def test_session_exceeding_max_is_revoked(self, db, test_user):
        """Session older than the absolute maximum should be rejected."""
        raw, row = create_refresh_token(db, test_user.id)
...
class TestCleanupExpiredTokens:
    ...
    def test_deletes_old_revoked_tokens(self, db, test_user):
        """Revoked tokens older than 24 hours should be cleaned up."""
    ...
    def test_keeps_active_tokens(self, db, test_user):
        """Active tokens should not be cleaned up."""
