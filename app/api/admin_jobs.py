...
async def get_job_schedule(admin: dict = Depends(require_admin)):
    ...
    scheduled_jobs = [
        ...
        ScheduledJob(
            ...
            next_run=(
                now + timedelta(days=1) - timedelta(hours=now.hour - 2)
            ).isoformat(),
            enabled=True,
        ),
        ScheduledJob(
    ...
    return JobScheduleResponse(
        scheduled_jobs=scheduled_jobs, total_jobs=len(scheduled_jobs)
    )


@router.post("/jobs/trigger/{job_name}", response_model=JobTriggerResponse)
async def trigger_job(
    job_name: str, request: JobTriggerRequest, admin: dict = Depends(require_admin)
):
    """
    ...
    task = celery_app.send_task(
        job_name, kwargs=request.job_parameters, queue="admin_jobs"
    )
