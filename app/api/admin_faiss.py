...
async def build_index(request: IndexBuildRequest, admin: dict = Depends(require_admin)):
    """
    ...
    task = celery_app.send_task(
        "build_faiss_index",
        args=[request.embedding_batch_paths, request.index_type, new_version],
        kwargs={
...
async def hot_swap_index(
    request: IndexHotSwapRequest, admin: dict = Depends(require_admin)
):
    """
...
async def get_index_status(admin: dict = Depends(require_admin), version: str = None):
    """
    ...
    if version and version != current_index_metadata["version"]:
        raise HTTPException(
            status_code=404, detail=f"Index version '{version}' not found"
        )
    ...
    return IndexStatusResponse(
        ...
        performance_metrics=PerformanceMetrics(
            **current_index_metadata["performance_metrics"]
        ),
