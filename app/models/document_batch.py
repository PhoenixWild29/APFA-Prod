...
class DocumentBatch(BaseModel):
    ...
    documents: List[str] = Field(
        ..., description="List of document IDs or paths", min_length=1, max_length=1000
    )
    batch_size: int = Field(
        1000, description="Number of documents in batch", ge=1, le=1000
    )
    priority: int = Field(
        9, description="Processing priority (1=lowest, 10=highest)", ge=1, le=10
    )
    ...
    def validate_documents_not_empty(cls, v: List[str]) -> List[str]:
        ...
        for idx, doc in enumerate(v):
            if not doc or not doc.strip():
                raise ValueError(f"Empty documents not allowed (found at index {idx})")
        return v
