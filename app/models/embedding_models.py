...
class EmbeddingBatch(BaseModel):
    ...
    document_count: int = Field(
        ..., description="Number of documents in batch", ge=1, le=1000
    )
    embedding_dimensions: int = Field(
        384, description="Vector dimensionality (all-MiniLM-L6-v2)", ge=1
    )
    ...
    minio_path: str = Field(
        ..., description="Path to batch file in MinIO", min_length=1
    )
    checksum: str = Field(
        ...,
...
class VectorEmbedding(BaseModel):
    ...
    document_id: str = Field(
        ..., description="Document identifier", min_length=1, max_length=255
    )
    embedding: List[float] = Field(..., description="384-dimensional embedding vector")
    normalized: bool = Field(True, description="Whether vector is L2-normalized")
    model_version: str = Field(
        "all-MiniLM-L6-v2", description="Embedding model version"
    )
    processing_time_ms: float = Field(
        ..., description="Time taken to generate embedding (ms)", ge=0.0
    )
