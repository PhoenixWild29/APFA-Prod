...
class DocumentMetadata(BaseModel):
    ...
    file_type: str = Field(..., description="File MIME type", max_length=100)
    ...
    uploaded_by_user_id: str = Field(
        ..., description="User ID who uploaded the file", min_length=1, max_length=255
    )
    checksum: Optional[str] = Field(
        None, description="File checksum (MD5 or SHA256)", max_length=128
    )
...
class UploadStatus(BaseModel):
    ...
    current_state: UploadState = Field(..., description="Current processing state")
    progress_percentage: float = Field(
        default=0.0, description="Processing progress (0.0-100.0)", ge=0.0, le=100.0
    )
    ...
    error_message: Optional[str] = Field(
        None, description="Error message if failed", max_length=1000
    )
    failure_reason: Optional[str] = Field(
        None, description="Detailed failure reason", max_length=2000
    )
