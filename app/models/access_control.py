...
class AccessRequest(BaseModel):
    ...
    resource: str = Field(
        ..., description="Resource being accessed", min_length=1, max_length=255
    )
    action: str = Field(
        ..., description="Action being attempted", min_length=1, max_length=100
    )
...
class AccessDecision(BaseModel):
    ...
    request_id: str = Field(
        ..., description="Unique request identifier", min_length=1, max_length=255
    )
    user_id: str = Field(
    ...
    action: str = Field(
        ..., description="Action attempted", min_length=1, max_length=100
    )
    decision: Literal["allow", "deny"] = Field(
        ..., description="Access decision (allow or deny)"
    )
    reason: str = Field(..., description="Reason for the decision", max_length=1000)
    applied_policies: List[str] = Field(
        default_factory=list, description="List of policies applied in decision"
    )
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
