...
class ResponseMetrics(BaseModel):
    ...
    total_latency_ms: float = Field(
        ..., description="Total end-to-end response time (ms)", ge=0.0
    )
    rag_retrieval_ms: float = Field(
        ..., description="RAG retrieval duration (ms)", ge=0.0
    )
    llm_inference_ms: float = Field(
        ..., description="LLM inference duration (ms)", ge=0.0
    )
    cache_lookup_ms: float = Field(
        ..., description="Cache lookup duration (ms)", ge=0.0
    )
    agent_coordination_ms: float = Field(
        ..., description="Agent coordination overhead (ms)", ge=0.0
    )
    was_cached: bool = Field(..., description="Whether response was served from cache")
    cache_hit_rate: float = Field(
        ..., description="Overall cache hit rate (0.0-1.0)", ge=0.0, le=1.0
    )
...
class CacheMetadata(BaseModel):
    ...
    cache_key: str = Field(
        ..., description="Unique cache key", min_length=1, max_length=255
    )
    cache_level: Literal["memory", "redis", "miss"] = Field(
        ..., description="Cache level"
    )
    hit_rate: float = Field(..., description="Cache hit rate (0.0-1.0)", ge=0.0, le=1.0)
    ttl_remaining_seconds: int = Field(..., description="Time to live remaining", ge=0)
    cache_size_bytes: int = Field(
        ..., description="Size of cached entry in bytes", ge=0
    )
...
class CacheInteraction(BaseModel):
    ...
    cache_level: Literal["memory", "redis"] = Field(..., description="Cache level")
...
class CachedAdviceResponse(BaseModel):
    ...
    advice_content: str = Field(
        ..., description="Generated advice content", min_length=1
    )
    ...
    cache_ttl_seconds: int = Field(
        600, description="Cache time-to-live (default 10 minutes)", ge=0
    )
