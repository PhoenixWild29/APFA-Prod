...
class DocumentProcessingEvent(BaseModel):
    ...
    ] = Field(..., description="Type of processing event")
    ...
    progress_percentage: float = Field(
        ..., description="Processing progress (0.0-100.0)", ge=0.0, le=100.0
    )
    processing_time_ms: Optional[float] = Field(
        None, description="Processing duration in milliseconds", ge=0.0
    )
...
class WebSocketDocumentMessage(BaseModel):
    ...
    document_id: Optional[str] = Field(
        None, description="Document identifier", max_length=255
    )
    batch_id: Optional[str] = Field(
        None, description="Batch identifier", max_length=255
    )
