...
class LoginEvent(BaseModel):
    ...
    event_type: LoginEventType = Field(..., description="Type of login event")
    ...
    failure_reason: Optional[str] = Field(
        None, description="Reason for login failure", max_length=500
    )
    security_score: float = Field(
        ..., description="Risk score (0.0-1.0, higher = riskier)", ge=0.0, le=1.0
    )
...
class WebSocketLoginMessage(BaseModel):
    ...
    message_type: MessageType = Field(..., description="Type of WebSocket message")
    event_data: LoginEvent = Field(..., description="Login event details")
    ...
    requires_admin_attention: bool = Field(
        default=False, description="Whether to alert administrators"
    )
        ...
        json_schema_extra = {
            "example": {
                ...
                "event_data": {
                    ...
                    "security_score": 0.85,
                    "geographic_location": {"country": "Unknown", "city": "Unknown"},
                },
                "risk_assessment": {
                    "threat_level": "high",
                    "reasons": [
