...
class BiasDetectionResults(BaseModel):
    ...
    bias_score: float = Field(
        ..., description="Overall bias score (0.0-1.0, lower is better)", ge=0.0, le=1.0
    )
    ...
    detected_issues: List[str] = Field(
        default_factory=list, description="List of detected bias issues"
    )
    mitigation_applied: List[str] = Field(
        default_factory=list, description="List of mitigation strategies applied"
    )
...
class OptimizedAdviceResponse(BaseModel):
    ...
    processing_time_ms: float = Field(
        ..., description="Total processing time (ms)", ge=0.0
    )
    was_cached: bool = Field(..., description="Whether response was served from cache")
    cache_hit_rate: float = Field(
        ..., description="Current cache hit rate (0.0-1.0)", ge=0.0, le=1.0
    )
