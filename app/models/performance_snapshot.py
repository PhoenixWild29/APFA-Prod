...
class SystemResourceMetrics(BaseModel):
    ...
    cpu_utilization_percent: float = Field(
        ..., description="CPU usage percentage", ge=0.0, le=100.0
    )
    memory_utilization_percent: float = Field(
        ..., description="Memory usage percentage", ge=0.0, le=100.0
    )
    disk_io_utilization_percent: float = Field(
        ..., description="Disk I/O usage percentage", ge=0.0, le=100.0
    )
    network_throughput_mbps: float = Field(
        ..., description="Network throughput in Mbps", ge=0.0
    )
    database_connections: int = Field(
        ..., description="Active database connections", ge=0
    )
    cache_memory_usage_mb: float = Field(
        ..., description="Cache memory usage in MB", ge=0.0
    )
...
class PerformanceSnapshot(BaseModel):
    ...
    request_count: int = Field(..., description="Total requests processed", ge=0)
    response_time_p95_ms: float = Field(
        ..., description="95th percentile response time", ge=0.0
    )
    response_time_p99_ms: float = Field(
        ..., description="99th percentile response time", ge=0.0
    )
    error_rate_percent: float = Field(
        ..., description="Error rate percentage", ge=0.0, le=100.0
    )
    cache_hit_rate_percent: float = Field(
        ..., description="Cache hit rate percentage", ge=0.0, le=100.0
    )
    active_requests: int = Field(..., description="Currently active requests", ge=0)
