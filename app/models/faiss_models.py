...
class IndexPerformanceMetrics(BaseModel):
    ...
    p95_search_latency_ms: float = Field(
        ..., description="95th percentile search latency in milliseconds", ge=0.0
    )
    p99_search_latency_ms: float = Field(
        ..., description="99th percentile search latency in milliseconds", ge=0.0
    )
    memory_usage_percent: float = Field(
        ..., description="Memory usage as percentage of available", ge=0.0, le=100.0
    )
    ...
    search_throughput_qps: float = Field(
        ..., description="Search throughput in queries per second", ge=0.0
    )
...
class FAISSIndexMetadata(BaseModel):
    ...
    index_version: str = Field(
        ..., description="Index version identifier", min_length=1, max_length=50
    )
    index_type: Literal["IndexFlatIP", "IndexIVFFlat"] = Field(
        ..., description="FAISS index type"
    )
    vector_count: int = Field(..., description="Number of vectors in index", ge=0)
    dimensions: int = Field(..., description="Vector dimensionality", ge=1)
    memory_size_mb: float = Field(
        ..., description="Index memory size in megabytes", ge=0.0
    )
    build_time_seconds: float = Field(
        ..., description="Time taken to build index in seconds", ge=0.0
    )
    performance_metrics: IndexPerformanceMetrics = Field(
        ..., description="Performance tracking metrics"
    )
    hot_swap_status: str = Field(
        ..., description="Current hot-swap status", max_length=50
    )
...
class MigrationTriggers(BaseModel):
    ...
    vector_count_threshold: int = Field(
        500000, description="Trigger migration at this vector count", ge=0
    )
    p95_latency_threshold_ms: float = Field(
        200.0, description="P95 latency threshold for migration", ge=0.0
    )
    memory_threshold_percent: float = Field(
        50.0, description="Memory usage threshold for migration", ge=0.0, le=100.0
    )
    faiss_latency_threshold_percent: float = Field(
        20.0, description="FAISS latency % threshold for migration", ge=0.0, le=100.0
    )
    early_warning_vector_count: int = Field(
        400000, description="Early warning vector count threshold", ge=0
    )
    early_warning_latency_ms: float = Field(
        100.0, description="Early warning latency threshold", ge=0.0
    )
    growth_rate_projection_days: int = Field(
        30, description="Days to project growth for proactive scaling", ge=1
    )
...
class EnhancedIndexPerformanceMetrics(BaseModel):
    ...
    def check_migration_triggers(self):
        ...
        if (
            self.faiss_latency_percent_of_request
            > triggers.faiss_latency_threshold_percent
        ):
            reasons.append(
...
class FAISSIndex(BaseModel):
    ...
    migration_triggers: MigrationTriggers = Field(
        default_factory=MigrationTriggers, description="Migration trigger thresholds"
    )
...
class MigrationReadiness(BaseModel):
    ...
    blocking_issues: List[str] = Field(
        default_factory=list, description="Issues blocking migration"
    )
    readiness_score: float = Field(
        ..., description="Readiness score (0.0-1.0)", ge=0.0, le=1.0
    )
...
class EnhancedIndexMetadata(BaseModel):
    ...
    migration_readiness: MigrationReadiness = Field(
        ..., description="Migration readiness"
    )
    ...
    def validate_score_range(cls, v: List[float]) -> List[float]:
        ...
        for score in v:
            if not 0.0 <= score <= 1.0:
                raise ValueError(
                    f"Similarity score {score} must be between 0.0 and 1.0"
                )
        return v
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "retrieval_time_ms": 45.2,
                "index_version": "v42",
                "search_parameters": {"top_k": 5, "similarity_threshold": 0.7},
                "performance_metrics": {
                    "cache_hit_rate": 0.75,
