...
class EmbeddingBatchTask(BaseModel):
    ...
    batch_size: int = Field(
        1000, description="Number of documents per batch", ge=1, le=1000
    )
    ...
    model_version: str = Field(
        "all-MiniLM-L6-v2", description="Embedding model version"
    )
    ...
    def validate_batch_size(cls, v: int, info) -> int:
        ...
        if document_ids and len(document_ids) != v:
            raise ValueError(
                f"Batch size ({v}) doesn't match document count ({len(document_ids)})"
            )
        return v
...
class IndexBuildTask(BaseModel):
    ...
    scheduled_build: bool = Field(
        False, description="Whether this is a scheduled rebuild"
    )
    trigger_type: str = Field("on-demand", description="Build trigger type")
    optimization_options: Dict[str, Any] = Field(
        default_factory=dict, description="Index optimization options"
    )
...
class TaskStatus(BaseModel):
    ...
    @classmethod
    def validate_completion_after_start(
