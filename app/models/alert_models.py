...
class AlertRule(BaseModel):
    ...
    rule_id: str = Field(
        ..., description="Unique rule identifier", min_length=1, max_length=100
    )
    name: str = Field(
        ..., description="Human-readable rule name", min_length=1, max_length=255
    )
    condition: str = Field(..., description="Alert condition expression", min_length=1)
    ...
    notification_channels: List[str] = Field(
        default_factory=list, description="Notification delivery channels"
    )
    cooldown_seconds: int = Field(
        300, description="Minimum time between alerts (default 5 minutes)", ge=0
    )
...
class AlertEvent(BaseModel):
    ...
    alert_id: str = Field(
        ..., description="Unique alert identifier", min_length=1, max_length=100
    )
    rule_id: str = Field(
        ..., description="Associated rule identifier", min_length=1, max_length=100
    )
    ...
    @classmethod
    def validate_resolution_after_trigger(
