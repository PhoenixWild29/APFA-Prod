...
def analyze_linguistic_features(query: str) -> Dict[str, Any]:
    ...
    structure = (
        "complex" if len(sentences) > 1 or len(query.split(",")) > 2 else "simple"
    )
    ...
    if "," in query:
        complexity += 2
    if any(
        word in query_lower for word in ["however", "although", "because", "therefore"]
    ):
        complexity += 2
    if len(sentences) > 1:
        complexity += 2
...
def assess_query_complexity(
    query: str, entities: List[Dict], intent: Dict
) -> Dict[str, Any]:
...
def analyze_query_comprehensive(query: str) -> Dict[str, Any]:
    ...
    if len(entities) == 0:
        suggestions.append(
            "Include specific details (amounts, rates, loan types) for more accurate advice"
        )
