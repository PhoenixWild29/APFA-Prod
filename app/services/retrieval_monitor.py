...
def update_retrieval_metrics(search_latency_ms: float, cache_hit: bool, success: bool):
    ...
    retrieval_metrics["avg_search_latency_ms"] = (
        current_avg * (total - 1) + search_latency_ms
    ) / total
    ...
    if cache_hit:
        ...
        retrieval_metrics["cache_hit_rate"] = (
            current_rate * (total - 1) + 100.0
        ) / total
    else:
        current_rate = retrieval_metrics["cache_hit_rate"]
        retrieval_metrics["cache_hit_rate"] = current_rate * (total - 1) / total
        ...
        while True:
            ...
            event_data = {
                ...
                "optimization_recommendations": [
                    (
                        "Cache hit rate good"
                        if retrieval_metrics["cache_hit_rate"] > 80
                        else "Consider increasing cache size"
                    ),
                    (
                        "Search latency optimal"
                        if retrieval_metrics["avg_search_latency_ms"] < 100
                        else "Review FAISS index optimization"
                    ),
                ],
            }
