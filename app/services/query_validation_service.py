...
def calculate_financial_relevance_score(query: str) -> float:
    ...
    high_count = sum(
        1 for keyword in FINANCIAL_KEYWORDS["high_relevance"] if keyword in query_lower
    )
    ...
    medium_count = sum(
        1
        for keyword in FINANCIAL_KEYWORDS["medium_relevance"]
    ...
    low_count = sum(
        1 for keyword in FINANCIAL_KEYWORDS["low_relevance"] if keyword in query_lower
    )
...
def validate_query_comprehensive(query: str) -> Dict[str, Any]:
    ...
    if profanity_detected:
        ...
        suggested_improvements.append(
            "Please rephrase your query using professional language"
        )
        explanatory_messages.append(
            f"Profanity detection confidence: {profanity_prob:.2%}"
        )
    ...
    if not is_financially_relevant:
        ...
        suggested_improvements.append(
            "Include specific financial terms (e.g., loan, mortgage, interest rate)"
        )
        explanatory_messages.append(
            f"Financial relevance score: {financial_relevance_score:.1f}/100"
        )
    else:
        explanatory_messages.append(
            f"Financial relevance score: {financial_relevance_score:.1f}/100 - Good"
        )
    ...
    if has_html or has_script:
        validation_failures.append(
            "Query contains potentially unsafe HTML or script content"
        )
    ...
    if is_too_long:
        ...
        suggested_improvements.append(
            "Shorten your query to focus on the main question"
        )
    ...
    is_valid = (
        ...
        and is_financially_relevant
        and is_content_safe
        and is_properly_formatted
    )
