...
def generate_verification_token(username: str) -> str:
    ...
    token_data = {
        ...
        "exp": datetime.now(timezone.utc) + timedelta(hours=settings.email_verification_expiry_hours),
    }
...
async def change_password(
    ...
    On success, all refresh token families are revoked so every device must
    re-authenticate.
    """
    auth_header = request.headers.get("Authorization", "")
    if not auth_header.startswith("Bearer "):
