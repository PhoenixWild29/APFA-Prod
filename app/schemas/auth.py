...
class User(BaseModel):
    ...
    hashed_password: str = Field(
        ..., description="Bcrypt hashed password", min_length=1
    )
    disabled: bool = Field(
        default=False, description="Whether the user account is disabled"
    )
    full_name: Optional[str] = Field(
        None, description="Full name of the user", max_length=100
    )
...
class Token(BaseModel):
    ...
    access_token: str = Field(..., description="JWT access token string", min_length=1)
...
class TokenPayload(BaseModel):
    ...
    sub: str = Field(
        ..., description="Subject (username)", min_length=1, max_length=255
    )
    exp: datetime = Field(..., description="Expiration time (UTC, timezone-aware)")
    iat: datetime = Field(
    ...
    session_id: str = Field(
        ..., description="Associated session identifier", min_length=1, max_length=255
    )
