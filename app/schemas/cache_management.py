...
class CacheWarmRequest(BaseModel):
    ...
    queries: List[str] = Field(
        ..., min_length=1, max_length=1000, description="Queries to warm"
    )
...
class CacheWarmResponse(BaseModel):
    ...
    estimated_cache_impact_mb: float = Field(
        ..., description="Estimated cache size impact"
    )
    warming_time_ms: float = Field(..., description="Total warming time")
    failed_queries: List[str] = Field(
        default_factory=list, description="List of failed queries"
    )
...
class PerformanceReport(BaseModel):
    ...
    resource_utilization_trends: Dict[str, List[float]] = Field(
        ..., description="Resource trends"
    )
    threshold_violations: List[Dict[str, Any]] = Field(
        ..., description="Threshold violations"
    )
    suggested_config_changes: Dict[str, Any] = Field(
        ..., description="Configuration suggestions"
    )
