...
class ProcessingMetrics(BaseModel):
    ...
    average_processing_duration_seconds: float = Field(
...
class IndexPerformance(BaseModel):
    ...
    search_response_time_p95_ms: float = Field(
        ..., description="P95 search response time"
    )
    search_response_time_p99_ms: float = Field(
        ..., description="P99 search response time"
    )
    index_size_mb: float = Field(..., description="Index size in MB")
    vector_count: int = Field(..., description="Total vector count")
    similarity_search_accuracy_percent: float = Field(
...
class StorageUtilization(BaseModel):
    ...
    storage_by_document_type: Dict[str, float] = Field(
        ..., description="Storage by type in GB"
    )
