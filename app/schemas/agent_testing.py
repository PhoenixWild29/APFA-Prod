...
class RetrieverTestRequest(BaseModel):
    ...
    query_text: str = Field(
        ..., min_length=10, max_length=500, description="Test query text"
    )
    context_requirements: Dict[str, Any] = Field(
        default_factory=dict, description="Expected context requirements"
    )
...
class RetrievalQualityMetrics(BaseModel):
    ...
    avg_relevance_score: float = Field(
        ..., ge=0.0, le=1.0, description="Average relevance"
    )
...
class DiagnosticInfo(BaseModel):
    ...
    embedding_quality_indicators: Dict[str, float] = Field(
        ..., description="Embedding quality"
    )
...
class RetrieverTestResult(BaseModel):
    ...
    # Validation results
    retrieval_accuracy: float = Field(
        ..., ge=0.0, le=1.0, description="Retrieval accuracy"
    )
    quality_metrics: RetrievalQualityMetrics = Field(
        ..., description="Quality assessment"
    )

    # Performance results
    performance_benchmark: PerformanceBenchmark = Field(
