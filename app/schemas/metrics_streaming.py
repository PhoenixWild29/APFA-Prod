...
class AgentExecutionMetrics(BaseModel):
    ...
    orchestrator_avg_ms: float = Field(
        ..., description="Orchestrator average execution time"
    )
...
class MetricsStreamData(BaseModel):
    ...
    agent_metrics: AgentExecutionMetrics = Field(
        ..., description="Agent execution metrics"
    )
    index_performance: IndexPerformanceData = Field(
        ..., description="Index performance"
    )
    celery_statistics: CeleryTaskStatistics = Field(
        ..., description="Celery statistics"
    )
