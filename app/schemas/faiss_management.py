...
class IndexOptimizationOptions(BaseModel):
    ...
    normalize_vectors: bool = Field(
        True, description="Normalize vectors before indexing"
    )
...
class IndexBuildRequest(BaseModel):
    ...
    embedding_batch_paths: List[str] = Field(
        ..., description="Paths to embedding batches"
    )
    index_type: Literal["IndexFlatIP", "IndexIVFFlat"] = Field(
        "IndexFlatIP", description="FAISS index type"
    )
    optimization_options: IndexOptimizationOptions = Field(
        default_factory=IndexOptimizationOptions
    )
...
class MigrationStatus(BaseModel):
    ...
    target_version: Optional[str] = Field(
        None, description="Target version for migration"
    )
    traffic_percentage: float = Field(
        0.0, description="Traffic percentage on new index"
    )
...
class IndexStatusResponse(BaseModel):
    ...
    migration_thresholds: Dict[str, Any] = Field(
        default_factory=dict, description="Migration trigger thresholds"
    )
...
class IndexHotSwapRequest(BaseModel):
    ...
    gradual_migration: bool = Field(
        True, description="Enable gradual traffic migration"
    )
    rollback_on_regression: bool = Field(
        True, description="Auto-rollback on performance regression"
    )
    latency_regression_threshold: float = Field(
        10.0, description="Max latency increase % before rollback"
    )
...
class IndexHotSwapResponse(BaseModel):
    ...
    validation_results: Dict[str, bool] = Field(
        ..., description="Pre-deployment validation results"
    )
