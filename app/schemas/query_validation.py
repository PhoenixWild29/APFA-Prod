...
class QueryValidationRequest(BaseModel):
    ...
    query: str = Field(
        ..., min_length=1, max_length=1000, description="Query text to validate"
    )
...
class QueryValidationResponse(BaseModel):
    ...
    is_financially_relevant: bool = Field(
        ..., description="Whether query is financially relevant"
    )
    is_content_safe: bool = Field(..., description="Whether content is safe")
    is_properly_formatted: bool = Field(
        ..., description="Whether query is properly formatted"
    )

    # Scores and metrics
    financial_relevance_score: float = Field(
        ..., ge=0.0, le=100.0, description="Financial relevance score (0-100)"
    )
    ...
    suggested_improvements: List[str] = Field(
        default_factory=list, description="Suggested query improvements"
    )
    explanatory_messages: List[str] = Field(
        default_factory=list, description="Detailed explanatory messages"
    )
