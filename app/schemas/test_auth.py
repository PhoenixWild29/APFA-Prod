...
def test_user_username_validation():
    ...
    # Valid usernames
    valid_user = User(username="valid_user-123", hashed_password="$2b$12$test")
...
def test_token_payload_expiration_validation():
    ...
    try:
        TokenPayload(sub="user4", exp=past_time, session_id="sess_abc")
        assert False, "Should have raised validation error"
    except Exception as e:
...
def test_token_payload_timezone_aware():
    ...
    payload = TokenPayload(
        sub="user5", exp=naive_exp, iat=naive_iat, session_id="sess_xyz"
    )
...
def test_token_payload_jti_validation():
    ...
    payload = TokenPayload(
        sub="user6", exp=future_time, jti=valid_uuid, session_id="sess_123"
    )
    ...
    try:
        TokenPayload(
            sub="user7", exp=future_time, jti="not-a-valid-uuid", session_id="sess_456"
        )
        assert False, "Should have raised validation error"
    except Exception as e:
...
def test_json_serialization():
    ...
    user = User(username="test_user", hashed_password="$2b$12$test", disabled=False)

    token = Token(access_token="test_token_string", token_type="bearer")
