...
class AgentStatusInfo(BaseModel):
    ...
    health_status: Literal["healthy", "unhealthy", "degraded"] = Field(
        ..., description="Health status"
    )
    processing_capability: Literal["available", "busy", "offline"] = Field(
        ..., description="Processing capability"
    )
    performance_metrics: AgentPerformanceMetrics = Field(
        ..., description="Performance metrics"
    )
    last_execution_timestamp: Optional[str] = Field(
...
class AgentTestRequest(BaseModel):
    ...
    agent_name: Optional[Literal["retriever", "analyzer", "orchestrator", "all"]] = (
        Field("all", description="Agent to test (or 'all')")
    )
...
class AgentTestResult(BaseModel):
    ...
    performance_benchmark: Optional[Dict[str, float]] = Field(
...
class AgentTestResponse(BaseModel):
    ...
    individual_results: List[AgentTestResult] = Field(
