...
class LinguisticAnalysis(BaseModel):
    ...
    financial_terminology_density: float = Field(
        ..., ge=0.0, le=1.0, description="Financial term density"
    )
    grammatical_complexity: int = Field(
        ..., ge=1, le=10, description="Grammar complexity (1-10)"
    )
...
class ComplexityAssessment(BaseModel):
    ...
    vocabulary_complexity: int = Field(
        ..., ge=1, le=10, description="Vocabulary difficulty"
    )
    multi_part_questions: int = Field(..., description="Number of sub-questions")
    required_domain_knowledge: int = Field(
        ..., ge=1, le=10, description="Required expertise level"
    )
...
class ProcessingRecommendations(BaseModel):
    ...
    estimated_processing_time_ms: float = Field(
...
class QueryAnalysisMetadata(BaseModel):
    ...
    suggested_improvements: List[str] = Field(
        ..., description="Query improvement suggestions"
    )
