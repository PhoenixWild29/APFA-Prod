...
class AdviceStatusResponse(BaseModel):
    ...
    progress_percentage: float = Field(
        ..., ge=0.0, le=100.0, description="Processing progress (0-100%)"
    )
    estimated_completion_time: Optional[str] = Field(
        None, description="Estimated completion time (ISO format)"
    )
    current_processing_stage: str = Field(..., description="Current processing stage")
    stage_specific_metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Stage-specific processing metadata"
    )
