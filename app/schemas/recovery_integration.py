...
class RetryFailedRequest(BaseModel):
    ...
    error_type_filter: List[str] = Field(
        default_factory=list, description="Filter by error types (empty = retry all)"
    )
    max_retry_attempts: int = Field(
        3, ge=1, le=10, description="Max retry attempts per document"
    )
...
class CeleryStatusResponse(BaseModel):
    ...
    task_execution_stats: Dict[str, Any] = Field(
    ...
    workers: List[CeleryWorkerStatus] = Field(
        ..., description="Individual worker status"
    )
...
class MinIOStatusResponse(BaseModel):
    ...
    bucket_health_status: Dict[str, str] = Field(
        ..., description="Health status per bucket"
    )
    data_integrity_checks: Dict[str, bool] = Field(
        ..., description="Data integrity results"
    )
