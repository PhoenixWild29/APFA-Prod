...
class AgentPerformanceAnalysis(BaseModel):
    ...
    success_rates: Dict[str, float] = Field(
        ..., description="Success rates by time window"
    )

    # Error patterns
    error_patterns: List[ErrorPattern] = Field(
        ..., description="Categorized error analysis"
    )

    # Historical trends
    historical_trends: List[HistoricalTrend] = Field(
...
class AgentConfigurationUpdate(BaseModel):
    ...
    agent_name: Literal["retriever", "analyzer", "orchestrator"] = Field(
    ...
    timeout_seconds: Optional[float] = Field(
        None, ge=1.0, le=300.0, description="Timeout value"
    )
    max_retries: Optional[int] = Field(
        None, ge=0, le=10, description="Max retry attempts"
    )
    performance_tuning: Optional[Dict[str, Any]] = Field(
        None, description="Performance parameters"
    )
